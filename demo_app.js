(function () {
  ...
  /* ---------------- demo seed data (preview only) ---------------- */
  function seedConversations() {
    var now = Date.now();
    var day = 86400000;
    return [
      {
        id: "demo-1", name: "Alex", avatarImage: null, avatarColor: "#5AC8FA",
        updatedAt: now - 5 * 60000,
        messages: [
          { id: "m1", text: "I made your favorite pasta tonight, wish you were here to make fun of my cooking.", timestamp: now - 3 * day, status: "delivered" },
          { id: "m2", text: "Saw the weirdest cloud today and thought of you", timestamp: now - 2 * day, status: "delivered" },
          { id: "m3", text: "I still have your coffee mug. Haven't been able to put it away.", timestamp: now - 20 * 60000, status: "delivered" },
          { id: "m4", text: "Some days are easier than others. Today's a hard one.", timestamp: now - 5 * 60000, status: "delivered" }
        ]
      },
      {
        id: "demo-2", name: "Sam", avatarImage: null, avatarColor: "#FF9500",
        updatedAt: now - 9 * day,
        messages: [
          { id: "m5", text: "Remember that terrible coffee place we used to go to every Sunday?", timestamp: now - 9 * day, status: "read" }
        ]
      },
      {
        id: "demo-3", name: "Jordan", avatarImage: null, avatarColor: "#AF52DE",
        updatedAt: now - 40 * 60000,
        messages: [
          { id: "m6", text: "hey", timestamp: now - 45 * 60000, status: "delivered" },
          { id: "m7", text: "just wanted to say I hope you're doing okay out there", timestamp: now - 44 * 60000, status: "delivered" },
          { id: "m8", text: "no pressure to reply, obviously", timestamp: now - 40 * 60000, status: "delivered" }
        ]
      }
    ];
  }
  ...
  /* ---------------- storage ---------------- */
  // DEMO MODE: the real app persists to localStorage; this preview keeps
  // everything in memory only (artifacts can't use browser storage), so a
  // refresh resets it back to the seed data below.
  function loadConversations() { return seedConversations(); }
  function saveConversations() { /* no-op in demo */ }
  ...
  function loadPinSettings() { return { enabled: false, hash: null, salt: null }; }
  function savePinSettings() { /* no-op in demo (in-memory only) */ }
  ...
  var demoUnlocked = false; // in-memory stand-in for sessionStorage in this preview
  function isUnlockedThisSession() { return demoUnlocked; }
  function setUnlockedThisSession(v) { demoUnlocked = v; }
  ...
  var demoDarkModePref = null; // in-memory stand-in for localStorage in this preview
  function loadDarkModePref() {
    if (demoDarkModePref !== null) return demoDarkModePref;
